...
class sarsaAgent():
    ...
    def __init__(self):
        ...
        self.n_pos = int((self.upper_bounds[0]-self.lower_bounds[0])/self.dp)
        self.n_vel = int((self.upper_bounds[1]-self.lower_bounds[1])/self.dv)
        self.weights_T1 = np.zeros([self.n_pos*self.n_vel,3])
        self.n_tiles = 5
        self.weights_T2 = np.zeros([self.n_pos*self.n_vel*self.n_tiles,3])
        self.p_offset = self.dp/self.n_tiles
        self.v_offset = self.dv/self.n_tiles
    ...
    def get_table_features(self, obs):
        pos = int((obs[0]-self.lower_bounds[0])//self.dp)
        vel = int((obs[1]-self.lower_bounds[1])//self.dv)
        state = np.zeros(self.n_pos*self.n_vel)
        state[pos*self.n_vel+vel] = 1
        return state
    ...
    def get_better_features(self, obs):
        state = np.zeros(self.n_pos*self.n_vel*self.n_tiles)
        for i in range(self.n_tiles):
            pos = int((obs[0]-self.lower_bounds[0] - i*self.p_offset)//self.dp)
            vel = int((obs[1]-self.lower_bounds[1] - i*self.v_offset)//self.dv)
            state[i*self.n_pos*self.n_vel+pos*self.n_vel+vel] = 1
        return state
    ...
    def choose_action(self, state, weights, epsilon):
        ...
        if decision_var<=epsilon:
            a = np.random.choice(np.arange(0,3))
        else:
    ...
    def sarsa_update(self, state, action, reward, new_state, new_action, learning_rate, weights):
        ...
        new_q = np.dot(new_state,weights)[new_action]
        weights[:,action] = weights[:,action] + learning_rate*( reward + self.discount*new_q - q)*state
        return weights
